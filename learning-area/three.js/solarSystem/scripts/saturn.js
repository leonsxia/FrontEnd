...
Saturn.prototype.createGlobe = function (map) {
	...
	var texture = new THREE.TextureLoader().load(saturnmap);
...
Saturn.prototype.createRings = function () {
	...
	var geometry = new THREE.RingGeometry(1.1, 1.867, 64);

	var texture = new THREE.TextureLoader().load(ringsmap);
	texture.colorSpace = THREE.SRGBColorSpace;
...
Saturn.Rings = function ( innerRadius, outerRadius, nSegments ) {
	...
	for (i = 0; i < iVer; i++) {
		...
		// 	       						new THREE.UV(1, 0),
		// 	       						new THREE.UV(0, 0) ] );
		uvArr.push(0,1,1,1,1,0,0,1,1,0,0,0);
	}	
	this.attributes.uv = new THREE.BufferAttribute(new Float32Array(uvArr), 2);
...
Saturn.Rings.prototype = new THREE.BufferGeometry();
